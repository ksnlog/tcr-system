...
const additionalItems = () => [
  {no:"2.1", desc:"Copper pipe With Insulation - Supply & Labour, Upto 2 Ton",       rate:335,  unit:"Ft", group:"low",  qty:0},
  {no:"2.2", desc:"Copper pipe With Insulation - Supply & Labour, 2.5/3/4 Ton",      rate:366,  unit:"Ft", group:"high", qty:0},
  {no:"3",   desc:"Supply and laying of Electrical 3/4 Core Cable",                  rate:49,   unit:"Ft", group:"all",  qty:0},
...
const numToWords = n => {
  ...
  const k=(n)=>{let s='';n=Math.floor(n);if(n>100){s=a[Math.floor(n/100)]+' Hundred ';n%=100;}if(n>19)s+=b[Math.floor(n/10)]+(n%10?' '+a[n%10]:'');else s+=a[n];return s.trim();};
  if(n>=10000000)return k(Math.floor(n/10000000))+' Crore '+k(Math.floor((n%10000000)/100000))+' Lakh '+k(Math.floor((n%100000)/1000))+' Thousand '+k(n%1000);
  if(n>=100000)return k(Math.floor(n/100000))+' Lakh '+k(Math.floor((n%100000)/1000))+' Thousand '+k(n%1000);
  if(n>=1000)return k(Math.floor(n/1000))+' Thousand '+k(n%1000);
  return k(n);
};
...
export default function App() {
  ...
  const [quotCust, setQuotCust]                 = useState({ name:'', mobile:'', address:'', gstin:'' });
  const [quotNo, setQuotNo]                     = useState('');
  const [quotSearchQ, setQuotSearchQ]           = useState('');
  const [quotValidDays, setQuotValidDays]       = useState('30');
  const [quotHistory, setQuotHistory]           = useState([]);
  const [showQuotHistory, setShowQuotHistory]   = useState(false);
  const [quotHistLoading, setQuotHistLoading]   = useState(false);
  const [quotMsg, setQuotMsg]                   = useState('');

  // ── Bill Register State ────────────────────────────────────────────────────
  const [billRegister, setBillRegister]         = useState([]);
  const [billRegSearch, setBillRegSearch]       = useState('');
  const [billRegLoading, setBillRegLoading]     = useState(false);
 ...
 function consumptionLabel(usageCount) {
  ...
  async function handleBulkSparesUpload(e) {
    ...
    if (!sfTabSession?.sfId) return setSpareMsg('Login to SF first');
    ...
    try {
      ...
      if (!items || items.length === 0) return setSpareMsg('No data found in file');
      const r = await fetch('/api/inventory/spares', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
      ...
      let d;
      try { d = JSON.parse(text); } catch { return setSpareMsg('Server error: ' + text.substring(0, 100)); }
      if (d.error) return setSpareMsg(d.error);
      setSfSpares(d.items || []);
  ...
  async function saveLocation(materialCode, location) {
    try {
      const r = await fetch('/api/inventory/spares', {
        method: 'POST', headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ sfId: sfTabSession.sfId, action: 'updateItem', materialCode, location })
      });
      const d = await r.json();
      setSfSpares(d.items || []);
    } catch(e) { setSpareMsg('Error saving location'); }
    setEditingLocation(null); setLocationDraft('');
  }

  function addInvoiceItem(spare) {
  if (parseFloat(spare.stock) <= 0) {
    setSpareMsg('❌ ' + spare.description + ' — Not in stock. Cannot bill.');
    setTimeout(() => setSpareMsg(''), 3000);
    return;
  }
  ...
  async function generateInvoicePdf() {
    ...
    const roundOff = Math.round((netAmount - total) * 100) / 100;
    const invoiceDate = new Date().toLocaleDateString('en-IN');
    const invoiceData = {
      invoiceNo: invNo, date: invoiceDate,
    ...
    // ── STEP 1: Import jsPDF and generate PDF FIRST (must stay close to user gesture) ──
    // Do NOT await any fetch calls before this — browsers block downloads
    // triggered after multiple async ticks away from the original click event.
    let pdfBlob = null;
    try {
      const { jsPDF } = await import('jspdf');
      const doc = new jsPDF({ unit:'mm', format:'a4' });
      const W = 210, M = 10, CW = 190;
      let y = 8;

      const companyName = (sfTabSession?.sfName || 'COMPANY NAME').toUpperCase();
      const companyAddr = sfTabSession?.address || '';
      const companyPhone = sfTabSession?.contact || '';
      const companyGst = sfTabSession?.gst || '';
      const companyState = sfTabSession?.stateCode || '';
      const companyPan = companyGst.length >= 12 ? companyGst.substring(2, 12) : (sfTabSession?.pan || '');

      // ── COMPANY HEADER (centered) ─────────────────────────────────────────
      doc.setTextColor(0, 0, 0);
      doc.setFont('helvetica', 'bold'); doc.setFontSize(15);
      doc.text(companyName, W / 2, y, { align: 'center' }); y += 6;

      doc.setFont('helvetica', 'normal'); doc.setFontSize(8);
      if (companyAddr) {
        const addrLines = doc.splitTextToSize(companyAddr, 170);
        addrLines.forEach(line => { doc.text(line, W / 2, y, { align: 'center' }); y += 4; });
      }
      if (companyPhone) { doc.text('PH.: ' + companyPhone, W / 2, y, { align: 'center' }); y += 4; }
      if (companyGst) {
        const gstLine = 'GSTIN : ' + companyGst
          + (companyState ? '  State : ' + companyState : '')
          + (companyPan ? '  PAN: ' + companyPan : '');
        doc.text(gstLine, W / 2, y, { align: 'center' }); y += 4;
      }
      ...
      // TAX INVOICE title + ORIGINAL label
      doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
      doc.text('TAX INVOICE', W / 2, y, { align: 'center' });
      doc.setFont('helvetica', 'normal'); doc.setFontSize(7);
      doc.text('ORIGINAL For Recipient', W - M, y, { align: 'right' });
      y += 3;
      doc.setLineWidth(0.4); doc.line(M, y, W - M, y); y += 2;

      // ── CUSTOMER BOX (left) + INVOICE DETAILS (right) ────────────────────
      const boxTop = y;
      const leftW = 115, rightW = CW - leftW, boxH = 50;

      doc.setDrawColor(100, 100, 100); doc.setLineWidth(0.3);
      doc.rect(M, boxTop, CW, boxH);
      doc.line(M + leftW, boxTop, M + leftW, boxTop + boxH); // vertical divider

      // Left: Customer details
      let ly = boxTop + 5;
      doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(0, 0, 0);
      doc.text('M/s ' + (invoiceCust.name || ''), M + 2, ly); ly += 4;
      doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5);
      if (invoiceCust.address) {
        const custAddrLines = doc.splitTextToSize(invoiceCust.address, leftW - 6);
        custAddrLines.slice(0, 4).forEach(line => { doc.text(line, M + 2, ly); ly += 3.5; });
      }
      doc.text('Ph. : ' + (invoiceCust.mobile || ''), M + 2, ly); ly += 4;
      if (invoiceCust.gstin) {
        doc.text('GSTIN/UIN : ' + invoiceCust.gstin, M + 2, ly); ly += 3.5;
        const stateCode = invoiceCust.gstin.substring(0, 2);
        doc.text('STATE     : ' + stateCode, M + 2, ly); ly += 3.5;
      }
      doc.setFontSize(7);
      doc.text('Tax is Payable on Reverse Charge Basis (Yes/No) : No', M + 2, boxTop + boxH - 3);

      // Right: Invoice fields
      const rx = M + leftW + 2;
      const rfx = M + leftW + rightW - 2; // right-align value x
      const ry0 = boxTop + 4;
      const rightFields = [
        ['Invoice No', invNo],
        ['Invoice Dt', invoiceDate],
        ['Due On', ''],
        ['Challan No', ''],
        ['Challan Dt', ''],
        ['Order No', ''],
      ...
      rightFields.forEach(([label, val], i) => {
        ...
        doc.setTextColor(0, 0, 0);
        if (val) doc.text(val, rfx, ry, { align: 'right' });
      });
      ...
      // Despatch From
      doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(0, 0, 0);
      doc.text('Despatch From:', M, y); y += 5;

      // ── ITEM TABLE ───────────────────────────────────────────────────────
      // Col widths: Sr=8, HSN/SAC=20, Desc=50, Qty=14, Unit=11, Rate=16, DiscAmt=15, TaxableAmt=18, %GST=10, IGST=14, Amount=14
      // Total = 8+20+50+14+11+16+15+18+10+14+14 = 190 ✓
      const cols =    [8,  20,  50,  14,  11,  16,  15,  18,  10,  14,  14];
      const colHdrs = ['Sr','HSN\n/SAC','Product Description','Qty','Unit','Rate','Disc\nAmt','Taxable\nAmt','%\nGST', invoiceTaxType==='inter'?'IGST':'Tax','Amount'];
      const hdrH = 9, rowH = 8;
      const tableTop = y;

      // Header background
      doc.setFillColor(20, 20, 20); doc.rect(M, tableTop, CW, hdrH, 'F');
      doc.setTextColor(255, 255, 255); doc.setFont('helvetica', 'bold'); doc.setFontSize(6.5);
      let hx = M;
      cols.forEach((cw, i) => {
        const cx = hx + cw / 2;
        const lines = colHdrs[i].split('\n');
        if (lines.length > 1) {
          doc.text(lines[0], cx, tableTop + 3.5, { align: 'center' });
          doc.text(lines[1], cx, tableTop + 7, { align: 'center' });
        } else {
          doc.text(lines[0], cx, tableTop + 5.5, { align: 'center' });
        }
        if (i < cols.length - 1) {
          doc.setDrawColor(80, 80, 80); doc.setLineWidth(0.2);
          doc.line(hx + cw, tableTop, hx + cw, tableTop + hdrH);
        }
        hx += cw;
      });
      y = tableTop + hdrH;
      ...
      // Item rows
      let totalTaxable = 0, totalTaxAmt = 0, totalQty = 0;
      invoiceItems.forEach((item, idx) => {
        const lt = (parseFloat(item.rate) || 0) * (parseFloat(item.qty) || 0);
        const disc = Math.min(parseFloat(item.discount) || 0, lt);
        const taxable = lt - disc;
        const taxAmtLine = Math.round(taxable * 0.18 * 100) / 100;

        const bg = idx % 2 === 0 ? 255 : 248;
        doc.setFillColor(bg, bg, bg); doc.rect(M, y, CW, rowH, 'F');
        doc.setTextColor(20, 20, 20); doc.setFont('helvetica', 'normal'); doc.setFontSize(7);
        ...
        const rowVals = [
          ...
          disc ? fmtINR(Math.round(disc)) : '0',
          fmtINR(Math.round(taxable * 100) / 100),
          '18',
          fmtINR(Math.round(taxAmtLine * 100) / 100),
          fmtINR(Math.round(taxable * 100) / 100),
        ];
        let rx2 = M;
        rowVals.forEach((val, i) => {
          const cw = cols[i];
          const isRight = i >= 3 && i !== 4;
          if (i === 2) {
            const descLines = doc.splitTextToSize(val, cw - 2);
            doc.text(descLines[0] || '', rx2 + 1, y + 5);
          } else if (isRight) {
            doc.text(val, rx2 + cw - 1, y + 5, { align: 'right' });
          } else {
            doc.text(val, rx2 + 1, y + 5);
          }
          if (i < cols.length - 1) {
            doc.setDrawColor(190, 190, 190); doc.setLineWidth(0.2);
            doc.line(rx2 + cw, y, rx2 + cw, y + rowH);
          }
          rx2 += cw;
        });
        doc.setDrawColor(210, 210, 210); doc.line(M, y + rowH, M + CW, y + rowH);
        totalTaxable += taxable;
        totalTaxAmt += taxAmtLine;
        totalQty += qty;
        y += rowH;
      });

      // Empty padding rows (minimum 3)
      const emptyRows = Math.max(3, 8 - invoiceItems.length);
      for (let i = 0; i < emptyRows; i++) {
        doc.setFillColor(255, 255, 255); doc.rect(M, y, CW, rowH, 'F');
        doc.setDrawColor(210, 210, 210); doc.line(M, y + rowH, M + CW, y + rowH);
        let ex = M;
        cols.forEach((cw, ci) => { ex += cw; if (ci < cols.length - 1) { doc.setDrawColor(210, 210, 210); doc.line(ex, y, ex, y + rowH); } });
        y += rowH;
      }

      // Table TOTAL row
      doc.setFillColor(235, 235, 235); doc.rect(M, y, CW, 7, 'F');
      doc.setDrawColor(100, 100, 100); doc.setLineWidth(0.3); doc.rect(M, y, CW, 7);
      doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.setTextColor(0, 0, 0);
      // Column x positions for numeric columns
      const colX = cols.reduce((acc, cw, i) => { acc.push((acc[i] || M) + (i === 0 ? 0 : cols[i-1])); return acc; }, [M]);
      // Qty total
      doc.text(totalQty.toFixed(3), colX[3] + cols[3] - 1, y + 5, { align: 'right' });
      // Taxable total
      doc.text(fmtINR(Math.round(totalTaxable * 100) / 100), colX[7] + cols[7] - 1, y + 5, { align: 'right' });
      // Tax total
      doc.text(fmtINR(Math.round(totalTaxAmt * 100) / 100), colX[9] + cols[9] - 1, y + 5, { align: 'right' });
      // Amount total (taxable, tax shown separately in summary)
      doc.text(fmtINR(Math.round(totalTaxable * 100) / 100), colX[10] + cols[10] - 1, y + 5, { align: 'right' });
      y += 7;

      // ── TAX SUMMARY BOX ──────────────────────────────────────────────────
      const taxBoxH = 16;
      doc.setFillColor(248, 250, 252); doc.setDrawColor(150, 150, 150); doc.setLineWidth(0.3);
      doc.rect(M, y, CW, taxBoxH, 'FD');
      doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.setTextColor(50, 50, 50);
      doc.text('GST%', M + 5, y + 5);
      doc.text('TAXABLE AMT', M + 32, y + 5);
      if (invoiceTaxType === 'inter') {
        doc.text('IGST', M + 95, y + 5);
        doc.setFont('helvetica', 'normal');
        doc.text('18.00%', M + 5, y + 11);
        doc.text('of  ' + fmtINR(Math.round(totalTaxable)) + '  =', M + 32, y + 11);
        doc.text(fmtINR(Math.round(totalTaxAmt)), M + 95, y + 11);
      } else {
        doc.text('CGST', M + 80, y + 5);
        doc.text('SGST', M + 115, y + 5);
        doc.setFont('helvetica', 'normal');
        doc.text('18.00%', M + 5, y + 11);
        doc.text('of  ' + fmtINR(Math.round(totalTaxable)) + '  =', M + 32, y + 11);
        doc.text(fmtINR(Math.round(totalTaxAmt / 2)), M + 80, y + 11);
        doc.text(fmtINR(Math.round(totalTaxAmt / 2)), M + 115, y + 11);
      }
      y += taxBoxH;

      // ── TOTALS ROWS ──────────────────────────────────────────────────────
      const labelX = M + 88;
      const col1X = W - M - 80, col2X = W - M - 50, col3X = W - M - 2;
      const drawTotalRow = (label, c1, c2, c3, bold, blue) => {
        const rh = 7;
        if (blue) { doc.setFillColor(225, 235, 255); doc.rect(M, y, CW, rh, 'F'); }
        doc.setDrawColor(150, 150, 150); doc.setLineWidth(0.3); doc.rect(M, y, CW, rh);
        doc.setFont('helvetica', bold ? 'bold' : 'normal');
        doc.setFontSize(bold ? 9 : 8);
        doc.setTextColor(blue ? 29 : 0, blue ? 78 : 0, blue ? 216 : 0);
        doc.text(label, labelX, y + 5);
        doc.setTextColor(0, 0, 0);
        if (c1) doc.text(c1, col1X, y + 5, { align: 'right' });
        if (c2) doc.text(c2, col2X, y + 5, { align: 'right' });
        doc.text(c3, col3X, y + 5, { align: 'right' });
        y += rh;
      };
      drawTotalRow('TOTAL', fmtINR(Math.round(totalTaxable)), fmtINR(Math.round(totalTaxAmt)), '0.00', false, false);
      drawTotalRow('Round Off', '', '', (roundOff >= 0 ? '+' : '') + Math.abs(roundOff).toFixed(2), false, false);
      ...
      const bankAcc = sfTabSession?.bankAcc || '';
      const bankName = sfTabSession?.bankName || '';
      if (bankName || bankIfsc)
        doc.text((bankName || 'BANK') + (bankIfsc ? '  IFSC-' + bankIfsc : ''), M, y);
      if (bankAcc)
        doc.text('A/C NO-' + bankAcc, W - M, y, { align: 'right' });
      y += 5;

      // Amount in Words
      doc.setFont('helvetica', 'bold'); doc.setFontSize(8);
      doc.text('[In Words] : ' + numToWords(netAmount) + ' Rupees Only.', M, y);
      y += 6;

      // ── TERMS ────────────────────────────────────────────────────────────
      doc.setDrawColor(150, 150, 150); doc.setLineWidth(0.2); doc.line(M, y, W - M, y); y += 3;
      doc.setFont('helvetica', 'italic'); doc.setFontSize(7); doc.setTextColor(90, 90, 90);
      doc.text('E. &. O. E.', M, y); y += 3.5;
      ...
      [
        ...
        '(4) Interest @12% p.a. will be charged, if invoice is not paid on or before due date.',
      ].forEach(t => { doc.text(t, M, y); y += 3.5; });

      // ── SIGNATURE ────────────────────────────────────────────────────────
      doc.setTextColor(0, 0, 0); doc.setFont('helvetica', 'bold'); doc.setFontSize(8);
      doc.text('For ' + companyName, W - M, y + 8, { align: 'right' });

      // Use blob URL download — more reliable than doc.save() across all browsers
      pdfBlob = doc.output('blob');
    } catch(e) {
      console.error('PDF generation error:', e);
      alert('PDF generation failed: ' + (e?.message || 'Unknown error. Please check the console.'));
    }

    // ── STEP 2: Trigger download immediately after PDF is built ──────────────
    if (pdfBlob) {
    ...
    // ── STEP 3: Backend saves (fire-and-forget, after download is triggered) ──
    try {
  ...
  function updateQuotLine(code, key, val) {
    setQuotItems(prev => prev.map(i => i.materialCode === code ? { ...i, [key]: parseFloat(val) || 0 } : i));
  }
  function removeQuotLine(code) {
    setQuotItems(prev => prev.filter(i => i.materialCode !== code));
  }
  function calcQuotTotals() {
    const subtotal = quotItems.reduce((s, i) => {
      const lineTotal = (parseFloat(i.rate) || 0) * (parseFloat(i.qty) || 0);
      const disc = Math.min(parseFloat(i.discount) || 0, lineTotal);
      return s + lineTotal - disc;
    }, 0);
  ...
  async function loadQuotHistory() {
    if (!sfTabSession?.sfId) return;
    setQuotHistLoading(true);
    try {
      const r = await fetch('/api/quotations?sfId=' + sfTabSession.sfId);
      const d = await r.json();
      setQuotHistory(d.quotations || []);
    } catch(e) {}
    setQuotHistLoading(false);
  }

  // ── Bill Register Functions ────────────────────────────────────────────────
  async function loadBillRegister() {
    if (!sfTabSession?.sfId) return;
    setBillRegLoading(true);
    try {
      const r = await fetch('/api/invoices?sfId=' + sfTabSession.sfId);
      const d = await r.json();
      setBillRegister(d.invoices || []);
    } catch(e) {}
    setBillRegLoading(false);
  }

  async function togglePayment(inv) {
    if (!sfTabSession?.sfId) return;
    try {
      const updates = { paymentReceived: !inv.paymentReceived };
  ...
  async function exportBillRegisterExcel() {
    try {
      ...
      const rows = billRegister.map(inv => {
        const taxable = (parseFloat(inv.subtotal) || 0);
        const taxAmt = (parseFloat(inv.taxAmt) || 0);
      ...
      utils.book_append_sheet(wb, ws, "Bill Register");
      writeFile(wb, `Bill_Register_${sfTabSession?.sfId || 'SF'}.xlsx`);
    } catch (err) {
      console.error(err);
  ...
  async function reprintInvoicePdf(inv) {
    try {
      const { jsPDF } = await import('jspdf');
      const doc = new jsPDF({ unit:'mm', format:'a4' });
      const W = 210, M = 10, CW = 190;
      ...
      const companyName = (inv.sfName || sfTabSession?.sfName || 'COMPANY NAME').toUpperCase();
      const companyAddr = sfTabSession?.address || '';
      const companyPhone = sfTabSession?.contact || '';
      const companyGst = sfTabSession?.gst || '';
      const companyState = sfTabSession?.stateCode || '';
      const companyPan = companyGst.length >= 12 ? companyGst.substring(2,12) : '';
      const invNo = inv.invoiceNo;
      const invoiceDate = inv.date;
      const invoiceTaxType = inv.taxType || 'intra';
      const invoiceCust = inv.customer || {};
      ...
      doc.setFont('helvetica','normal'); doc.setFontSize(8);
      if (companyAddr) {
        const al = doc.splitTextToSize(companyAddr,170);
        al.forEach(l=>{doc.text(l,W/2,y,{align:'center'}); y+=4;});
      }
      if (companyPhone) { doc.text('PH.: '+companyPhone, W/2, y, {align:'center'}); y+=4; }
      if (companyGst) {
        const gl = 'GSTIN : '+companyGst+(companyState?'  State : '+companyState:'')+(companyPan?'  PAN: '+companyPan:'');
        doc.text(gl, W/2, y, {align:'center'}); y+=4;
      }
      doc.setDrawColor(0,0,0); doc.setLineWidth(0.5); doc.line(M,y,W-M,y); y+=4;

      // TAX INVOICE title
      doc.setFont('helvetica','bold'); doc.setFontSize(10);
      doc.text('TAX INVOICE', W/2, y, {align:'center'});
      doc.setFont('helvetica','normal'); doc.setFontSize(7);
      doc.text('DUPLICATE / REPRINT', W-M, y, {align:'right'});
      y+=3; doc.setLineWidth(0.4); doc.line(M,y,W-M,y); y+=2;

      // ── CUSTOMER + INVOICE FIELDS BOX ────────────────────────────────────
      const boxTop=y, leftW=115, rightW=CW-leftW, boxH=50;
      doc.setDrawColor(100,100,100); doc.setLineWidth(0.3);
      doc.rect(M,boxTop,CW,boxH);
      doc.line(M+leftW,boxTop,M+leftW,boxTop+boxH);
      ...
      doc.setFont('helvetica','normal'); doc.setFontSize(7.5);
      if (invoiceCust.address) {
        const cl=doc.splitTextToSize(invoiceCust.address,leftW-6);
        cl.slice(0,4).forEach(l=>{doc.text(l,M+2,ly); ly+=3.5;});
      }
      doc.text('Ph. : '+(invoiceCust.mobile||''), M+2, ly); ly+=4;
      if (invoiceCust.gstin) {
        doc.text('GSTIN/UIN : '+invoiceCust.gstin, M+2, ly); ly+=3.5;
        doc.text('STATE     : '+invoiceCust.gstin.substring(0,2), M+2, ly); ly+=3.5;
      }
      doc.setFontSize(7);
      doc.text('Tax is Payable on Reverse Charge Basis (Yes/No) : No', M+2, boxTop+boxH-3);

      const rx=M+leftW+2, rfx=M+leftW+rightW-2, ry0=boxTop+4;
      const rightFields=[
        ['Invoice No',invNo],['Invoice Dt',invoiceDate],['Due On',''],
        ['Challan No',''],['Challan Dt',''],['Order No',''],
        ['Order Dt',''],['Transport',''],['L.R. No',''],['L.R. Dt',''],
      ];
      doc.setFontSize(7);
      rightFields.forEach(([label,val],i)=>{
        const ry=ry0+i*4.7;
        doc.setFont('helvetica','normal'); doc.setTextColor(60,60,60); doc.text(label+' :',rx,ry);
        doc.setTextColor(0,0,0); if(val) doc.text(val,rfx,ry,{align:'right'});
      });
      ...
      const cols=[8,20,50,14,11,16,15,18,10,14,14];
      const colHdrs=['Sr','HSN\n/SAC','Product Description','Qty','Unit','Rate','Disc\nAmt','Taxable\nAmt','%\nGST',invoiceTaxType==='inter'?'IGST':'Tax','Amount'];
      const hdrH=9, rowH=8, tableTop=y;

      doc.setFillColor(20,20,20); doc.rect(M,tableTop,CW,hdrH,'F');
      doc.setTextColor(255,255,255); doc.setFont('helvetica','bold'); doc.setFontSize(6.5);
      let hx=M;
      cols.forEach((cw,i)=>{
        const cx=hx+cw/2, lines=colHdrs[i].split('\n');
        if(lines.length>1){doc.text(lines[0],cx,tableTop+3.5,{align:'center'});doc.text(lines[1],cx,tableTop+7,{align:'center'});}
        else doc.text(lines[0],cx,tableTop+5.5,{align:'center'});
        if(i<cols.length-1){doc.setDrawColor(80,80,80);doc.setLineWidth(0.2);doc.line(hx+cw,tableTop,hx+cw,tableTop+hdrH);}
        hx+=cw;
      });
      y=tableTop+hdrH;
      ...
      invoiceItems.forEach((item,idx)=>{
        ...
        doc.setTextColor(20,20,20); doc.setFont('helvetica','normal'); doc.setFontSize(7);
        const qty=parseFloat(item.qty)||0;
        const rowVals=[String(idx+1),item.materialCode||'',item.description||'',qty.toFixed(3),item.unit||'Pc',
          fmtINR(parseFloat(item.rate)||0),disc?fmtINR(Math.round(disc)):'0',
          fmtINR(Math.round(taxable*100)/100),'18',fmtINR(Math.round(taxAmtLine*100)/100),fmtINR(Math.round(taxable*100)/100)];
        let rx2=M;
        rowVals.forEach((val,i)=>{
          const cw=cols[i], isRight=i>=3&&i!==4;
          if(i===2){const dl=doc.splitTextToSize(val,cw-2);doc.text(dl[0]||'',rx2+1,y+5);}
          else if(isRight) doc.text(val,rx2+cw-1,y+5,{align:'right'});
          else doc.text(val,rx2+1,y+5);
          if(i<cols.length-1){doc.setDrawColor(190,190,190);doc.setLineWidth(0.2);doc.line(rx2+cw,y,rx2+cw,y+rowH);}
          rx2+=cw;
        });
        doc.setDrawColor(210,210,210); doc.line(M,y+rowH,M+CW,y+rowH);
        totalTaxable+=taxable; totalTaxAmt+=taxAmtLine; totalQty+=qty; y+=rowH;
      });
      ...
      for(let i=0;i<emptyRows;i++){
        doc.setFillColor(255,255,255); doc.rect(M,y,CW,rowH,'F');
        doc.setDrawColor(210,210,210); doc.line(M,y+rowH,M+CW,y+rowH);
        let ex=M; cols.forEach((cw,ci)=>{ex+=cw;if(ci<cols.length-1){doc.setDrawColor(210,210,210);doc.line(ex,y,ex,y+rowH);}});
        y+=rowH;
      }

      // Totals row
      const netAmount=Math.round(inv.total||0);
      const roundOff=Math.round((netAmount-(totalTaxable+totalTaxAmt))*100)/100;
      doc.setFillColor(235,235,235); doc.rect(M,y,CW,7,'F');
      doc.setDrawColor(100,100,100); doc.setLineWidth(0.3); doc.rect(M,y,CW,7);
      doc.setFont('helvetica','bold'); doc.setFontSize(7.5); doc.setTextColor(0,0,0);
      const colX=cols.reduce((acc,cw,i)=>{acc.push((acc[i]||M)+(i===0?0:cols[i-1]));return acc;},[M]);
      ...
      doc.rect(M,y,CW,taxBoxH,'FD');
      doc.setFont('helvetica','bold'); doc.setFontSize(7.5); doc.setTextColor(50,50,50);
      doc.text('GST%',M+5,y+5); doc.text('TAXABLE AMT',M+32,y+5);
      if(invoiceTaxType==='inter'){
        doc.text('IGST',M+95,y+5); doc.setFont('helvetica','normal');
        doc.text('18.00%',M+5,y+11); doc.text('of  '+fmtINR(Math.round(totalTaxable))+'  =',M+32,y+11);
        doc.text(fmtINR(Math.round(totalTaxAmt)),M+95,y+11);
      } else {
        doc.text('CGST',M+80,y+5); doc.text('SGST',M+115,y+5); doc.setFont('helvetica','normal');
        doc.text('18.00%',M+5,y+11); doc.text('of  '+fmtINR(Math.round(totalTaxable))+'  =',M+32,y+11);
        doc.text(fmtINR(Math.round(totalTaxAmt/2)),M+80,y+11);
        doc.text(fmtINR(Math.round(totalTaxAmt/2)),M+115,y+11);
      }
      ...
      // Total rows
      const labelX=M+88, col1X=W-M-80, col2X=W-M-50, col3X=W-M-2;
      const drawTR=(label,c1,c2,c3,bold,blue)=>{
        const rh=7;
        if(blue){doc.setFillColor(225,235,255);doc.rect(M,y,CW,rh,'F');}
        doc.setDrawColor(150,150,150);doc.setLineWidth(0.3);doc.rect(M,y,CW,rh);
        doc.setFont('helvetica',bold?'bold':'normal');doc.setFontSize(bold?9:8);
        doc.setTextColor(blue?29:0,blue?78:0,blue?216:0);doc.text(label,labelX,y+5);
        doc.setTextColor(0,0,0);
        if(c1) doc.text(c1,col1X,y+5,{align:'right'});
        if(c2) doc.text(c2,col2X,y+5,{align:'right'});
        doc.text(c3,col3X,y+5,{align:'right'}); y+=rh;
      };
      drawTR('TOTAL',fmtINR(Math.round(totalTaxable)),fmtINR(Math.round(totalTaxAmt)),'0.00',false,false);
      ...
      doc.setFont('helvetica','bold'); doc.setFontSize(8); doc.setTextColor(0,0,0);
      if(bankName||bankIfsc) doc.text((bankName||'BANK')+(bankIfsc?'  IFSC-'+bankIfsc:''),M,y);
      if(bankAcc) doc.text('A/C NO-'+bankAcc,W-M,y,{align:'right'});
      y+=5;
      doc.text('[In Words] : '+numToWords(netAmount)+' Rupees Only.',M,y); y+=6;
      ...
      a.download='Invoice_REPRINT_'+invNo+'.pdf';
      document.body.appendChild(a); a.click();
      document.body.removeChild(a);
      setTimeout(()=>URL.revokeObjectURL(url),1000);
    } catch(e) {
      console.error('Reprint error:',e);
      alert('Reprint failed: '+(e?.message||'Unknown error'));
    }
  }

  async function generateQuotationPdf() {
    if (!quotCust.name) return alert('Customer name is required');
    if (quotItems.length === 0) return alert('Add at least one item');
    const { subtotal, taxAmt, total } = calcQuotTotals();
    const qNo = quotNo || ('QT-' + (sfTabSession?.sfId || '') + '-' + Date.now());
    const netAmount = Math.round(total);
    const roundOff = Math.round((netAmount - total) * 100) / 100;
    const quotDate = new Date().toLocaleDateString('en-IN');
    const validUntil = new Date(Date.now() + parseInt(quotValidDays||30) * 86400000).toLocaleDateString('en-IN');
    const quotData = {
      quotNo: qNo, date: quotDate, validUntil,
      customer: quotCust, sfName: sfTabSession?.sfName, sfId: sfTabSession?.sfId,
      items: quotItems, subtotal, taxAmt, total,
    };
    ...
    try {
      ...
      doc.setFont('helvetica', 'normal'); doc.setFontSize(7);
      doc.text('SUBJECT TO ACCEPTANCE', W - M, y, { align: 'right' });
      y += 3;
      doc.setLineWidth(0.4); doc.line(M, y, W - M, y); y += 2;

      // ── CUSTOMER BOX + QUOTATION FIELDS ──────────────────────────────────
      const boxTop = y;
      const leftW = 115, rightW = CW - leftW, boxH = 50;
      doc.setDrawColor(100, 100, 100); doc.setLineWidth(0.3);
      doc.rect(M, boxTop, CW, boxH);
      ...
      let ly = boxTop + 5;
      doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(0, 0, 0);
      doc.text('To,  ' + (quotCust.name || ''), M + 2, ly); ly += 4;
      doc.setFont('helvetica', 'normal'); doc.setFontSize(7.5);
      if (quotCust.address) {
        const custAddrLines = doc.splitTextToSize(quotCust.address, leftW - 6);
        custAddrLines.slice(0, 4).forEach(line => { doc.text(line, M + 2, ly); ly += 3.5; });
      }
      doc.text('Ph. : ' + (quotCust.mobile || ''), M + 2, ly); ly += 4;
      if (quotCust.gstin) {
        doc.text('GSTIN/UIN : ' + quotCust.gstin, M + 2, ly); ly += 3.5;
      }
      doc.setFontSize(7);
      doc.text('Kindly note this is a Quotation only — not a Tax Invoice.', M + 2, boxTop + boxH - 3);

      // Right: Quotation fields
      const rx = M + leftW + 2;
      const rfx = M + leftW + rightW - 2;
      const ry0 = boxTop + 4;
      const rightFields = [
        ['Quotation No', qNo],
        ['Quote Date', quotDate],
        ...
        ['Ref No', ''],
        ['Prepared By', sfTabSession?.sfName || ''],
        ['Contact', companyPhone || ''],
        ['Our GSTIN', companyGst || ''],
        ['Terms', 'Ex-Works'],
        ['Delivery', ''],
        ['Payment', 'Advance'],
      ];
      ...
      rightFields.forEach(([label, val], i) => {
        ...
        doc.text(label + ' :', rx, ry);
        doc.setTextColor(0, 0, 0);
        if (val) doc.text(String(val), rfx, ry, { align: 'right' });
      });

      y = boxTop + boxH + 4;

      doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(0, 0, 0);
      doc.text('Items / Services Quoted:', M, y); y += 5;

      // ── ITEM TABLE ────────────────────────────────────────────────────────
      const cols =    [8,  20,  60,  14,  11,  16,  15,  22,  24];
      const colHdrs = ['Sr','HSN\n/SAC','Product Description','Qty','Unit','Rate','Disc\nAmt','Taxable\nAmt','Amount\n(incl. 18% GST)'];
      const hdrH = 9, rowH = 8;
      const tableTop = y;
      ...
      let totalTaxable = 0, totalTaxAmt = 0, totalQty = 0;
      quotItems.forEach((item, idx) => {
        const lt = (parseFloat(item.rate) || 0) * (parseFloat(item.qty) || 0);
        const disc = Math.min(parseFloat(item.discount) || 0, lt);
        const taxable = lt - disc;
        const taxAmtLine = Math.round(taxable * 0.18 * 100) / 100;
        const lineTotal = taxable + taxAmtLine;
        ...
        const rowVals = [
          ...
          disc ? fmtINR(Math.round(disc)) : '0',
          fmtINR(Math.round(taxable * 100) / 100),
          fmtINR(Math.round(lineTotal * 100) / 100),
        ];
        let rx2 = M;
        rowVals.forEach((val, i) => {
          const cw = cols[i];
      ...
      // Empty padding rows
      const emptyRows = Math.max(3, 8 - quotItems.length);
      for (let i = 0; i < emptyRows; i++) {
        doc.setFillColor(255, 255, 255); doc.rect(M, y, CW, rowH, 'F');
        doc.setDrawColor(210, 210, 210); doc.line(M, y + rowH, M + CW, y + rowH);
        let ex = M;
      ...
      doc.text(totalQty.toFixed(3), colX[3] + cols[3] - 1, y + 5, { align: 'right' });
      doc.text(fmtINR(Math.round(totalTaxable * 100) / 100), colX[7] + cols[7] - 1, y + 5, { align: 'right' });
      doc.text(fmtINR(Math.round((totalTaxable + totalTaxAmt) * 100) / 100), colX[8] + cols[8] - 1, y + 5, { align: 'right' });
      y += 7;

      // ── GST SUMMARY BOX ───────────────────────────────────────────────────
      const taxBoxH = 16;
      doc.setFillColor(248, 252, 248); doc.setDrawColor(150, 150, 150); doc.setLineWidth(0.3);
      doc.rect(M, y, CW, taxBoxH, 'FD');
      doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.setTextColor(50, 50, 50);
      doc.text('GST% (Indicative)', M + 5, y + 5);
      doc.text('TAXABLE AMT', M + 55, y + 5);
      doc.text('CGST @9%', M + 102, y + 5);
      doc.text('SGST @9%', M + 138, y + 5);
      doc.text('TOTAL GST', M + 170, y + 5);
      doc.setFont('helvetica', 'normal');
      doc.text('18.00%', M + 5, y + 11);
      doc.text(fmtINR(Math.round(totalTaxable)), M + 55, y + 11);
      doc.text(fmtINR(Math.round(totalTaxAmt / 2)), M + 102, y + 11);
      doc.text(fmtINR(Math.round(totalTaxAmt / 2)), M + 138, y + 11);
      doc.text(fmtINR(Math.round(totalTaxAmt)), M + 170, y + 11);
      y += taxBoxH;

      // ── TOTALS ROWS ───────────────────────────────────────────────────────
      const labelX = M + 88;
      const col2X = W - M - 50, col3X = W - M - 2;
      const drawTotalRow = (label, c2, c3, bold, highlight) => {
        const rh = 7;
        if (highlight) { doc.setFillColor(220, 252, 231); doc.rect(M, y, CW, rh, 'F'); }
        doc.setDrawColor(150, 150, 150); doc.setLineWidth(0.3); doc.rect(M, y, CW, rh);
        doc.setFont('helvetica', bold ? 'bold' : 'normal');
        doc.setFontSize(bold ? 9 : 8);
        doc.setTextColor(highlight ? 22 : 0, highlight ? 101 : 0, highlight ? 52 : 0);
        doc.text(label, labelX, y + 5);
        doc.setTextColor(0, 0, 0);
        if (c2) doc.text(c2, col2X, y + 5, { align: 'right' });
        doc.text(c3, col3X, y + 5, { align: 'right' });
      ...
      drawTotalRow('SUB TOTAL (excl. GST)', '', fmtINR(Math.round(totalTaxable)), false, false);
      drawTotalRow('GST @ 18% (indicative)', '', fmtINR(Math.round(totalTaxAmt)), false, false);
      drawTotalRow('Round Off', '', (roundOff >= 0 ? '+' : '') + Math.abs(roundOff).toFixed(2), false, false);
      drawTotalRow('QUOTED AMOUNT (incl. GST)', '', fmtINR(netAmount), true, true);
      y += 5;
      ...
      [
        '(1) This quotation is valid for ' + (quotValidDays || '30') + ' days from the date of issue.',
        '(2) Prices are subject to change without prior notice after validity period.',
        '(3) GST shown is indicative at 18%. Actual GST will apply as per prevailing rates at time of invoice.',
        '(4) Delivery period to be confirmed at the time of order placement.',
        '(5) Payment terms: Advance unless otherwise agreed in writing.',
        '(6) This is a computer-generated quotation. Please quote this reference when placing an order.',
      ].forEach(t => { doc.text(t, M, y); y += 3.5; });

      // ── SIGNATURE ─────────────────────────────────────────────────────────
      doc.setTextColor(0, 0, 0); doc.setFont('helvetica', 'bold'); doc.setFontSize(8);
      doc.text('For ' + companyName, W - M, y + 10, { align: 'right' });
      doc.setFont('helvetica', 'normal'); doc.setFontSize(7);
      doc.text('Authorized Signatory', W - M, y + 17, { align: 'right' });
      doc.setDrawColor(80,80,80); doc.setLineWidth(0.3); doc.line(W - M - 50, y + 14, W - M, y + 14);
    ...
    // Save quotation record (no stock deduction)
    try {
      await fetch('/api/quotations', { method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ sfId: sfTabSession.sfId, quotation: quotData }) });
    } catch(e) { console.log('Quotation save error:', e); }

    setShowQuotation(false); setQuotItems([]);
    setQuotCust({ name:'', mobile:'', address:'', gstin:'' });
    setQuotNo(''); setQuotSearchQ('');
    setQuotMsg('✅ Quotation generated & downloaded!'); setTimeout(() => setQuotMsg(''), 3000);
  }
          ...
          <div style={{maxWidth:720,margin:'0 auto'}}>
            {!sfTabAuthed ? (
              ...
              <>
                ...
                {/* ─── BILL REGISTER SUB-TAB ─── */}
                {!sfTabDataLoading && sfSubTab==='billregister' && (
                  <div>
                    {/* Header */}
                    <div style={{background:'white',borderRadius:12,padding:14,marginBottom:12,boxShadow:'0 2px 8px rgba(0,0,0,.06)'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                        <div>
                          <div style={{fontSize:13,fontWeight:700,color:'#92400E'}}>🧾 Bill Register — {sfTabSession?.sfName}</div>
                          <div style={{fontSize:10,color:'#6B7280',marginTop:2}}>All tax invoices issued from the Spares tab. Read-only — reprint only.</div>
                        </div>
                        <div style={{display:'flex', gap: 6}}>
                          <button onClick={exportBillRegisterExcel}
                            style={{padding:'7px 12px',background:'#D1FAE5',border:'1.5px solid #A7F3D0',borderRadius:8,fontSize:11,fontWeight:600,color:'#065F46',cursor:'pointer'}}>
                            📤 Export Excel
                          </button>
                          <button onClick={loadBillRegister} disabled={billRegLoading}
                            style={{padding:'7px 12px',background:'#FEF3C7',border:'1.5px solid #FDE68A',borderRadius:8,fontSize:11,fontWeight:600,color:'#92400E',cursor:'pointer'}}>
                            {billRegLoading ? '…' : '↻ Refresh'}
                          </button>
                        </div>
                      </div>
                      ...
                      <div style={{position:'relative'}}>
                        <input value={billRegSearch} onChange={e=>setBillRegSearch(e.target.value)}
                          placeholder="🔍  Search by invoice number or customer name…"
                          style={{width:'100%',padding:'9px 12px',border:'1.5px solid #FDE68A',borderRadius:9,fontSize:12,outline:'none',background:'#FFFBEB',boxSizing:'border-box'}}/>
                        {billRegSearch && (
                          <button onClick={()=>setBillRegSearch('')}
                    ...
                    {/* Loading */}
                    {billRegLoading && <div style={{textAlign:'center',padding:40,color:'#9CA3AF',fontSize:13}}>Loading invoices…</div>}

                    {/* Empty */}
                    {!billRegLoading && billRegister.length===0 && (
                      <div style={{background:'white',borderRadius:12,padding:40,textAlign:'center',boxShadow:'0 2px 8px rgba(0,0,0,.06)'}}>
                        <div style={{fontSize:40,marginBottom:8}}>🧾</div>
                        <div style={{fontSize:13,fontWeight:600,color:'#374151',marginBottom:4}}>No invoices yet</div>
                        <div style={{fontSize:11,color:'#9CA3AF'}}>Invoices generated from the Spares tab will appear here</div>
                      </div>
                    )}
                          ...
                          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:12}}>
                            {[
                              ...
                              <div key={label} style={{background:bg,borderRadius:9,padding:'10px 8px',textAlign:'center'}}>
                                ...
                                <div style={{fontSize:9,fontWeight:600,color,marginTop:2}}>{label}</div>
                              </div>
                            ))}
                          </div>
                              ...
                              <table style={{width:'100%',borderCollapse:'collapse',fontSize:11,textAlign:'left'}}>
                                <thead>
                                  <tr style={{background:'#F3F4F6',borderBottom:'2px solid #E5E7EB'}}>
                                    <th style={{padding:'12px',fontWeight:700,color:'#374151',whiteSpace:'nowrap'}}>Invoice No</th>
                                    <th style={{padding:'12px',fontWeight:700,color:'#374151',whiteSpace:'nowrap'}}>Date</th>
                                  ...
                                  {displayInvoices.map(inv => (
                                    <tr key={inv.id} style={{borderBottom:'1px solid #E5E7EB'}}>
                                      ...
                                      <td style={{padding:'12px'}}>
                                        ...
                                        {inv.customer?.mobile && <div style={{fontSize:9,color:'#6B7280'}}>{inv.customer.mobile}</div>}
                                      </td>
                  ...
                  <div>
                    ...
                    {quotMsg && (
                      <div style={{background:'#F0FDF4',border:'1px solid #A7F3D0',
                        borderRadius:8,padding:'8px 12px',fontSize:12,color:'#065F46',marginBottom:10}}>
                        {quotMsg}
                      </div>
                    )}
                        ...
                        <div style={{marginBottom:10}}>
                          ...
                          {invoiceSearchQ.trim() && (
                            <div style={{border:'1px solid #E5E7EB',borderRadius:8,marginTop:4,maxHeight:160,overflowY:'auto',boxShadow:'0 4px 12px rgba(0,0,0,.08)'}}>
                              {sfSpares
  .filter(s => parseFloat(s.stock) > 0)
  .sort((a, b) => (b.usageCount || 0) - (a.usageCount || 0))
  .slice(0, 20)
  .filter(s => s.materialCode.toLowerCase().includes(invoiceSearchQ.toLowerCase()) || s.description.toLowerCase().includes(invoiceSearchQ.toLowerCase()))
  .map(s=>(
  ...
  .length === 0 && (
                      ...
                      /* ── Quotation builder (moved to ternary chain) ── */
                      null
                    ) : (
                      /* ── Spare Catalog View ── */
                      <>
                        ...
                        {sfSpares.length > 0 && (
                          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6,marginBottom:12}}>
                           {[
                            ...
                            <div style={{background:'#F0FDF4',borderRadius:8,padding:'8px 6px',textAlign:'center',gridColumn:'1/-1'}}>
                              <div style={{fontSize:14,fontWeight:700,color:'#16A34A',fontFamily:'monospace'}}>
                                ₹{fmtINR(sfSpares.reduce((sum,s)=>sum+(parseFloat(s.dp)||0)*(parseFloat(s.stock)||0),0))}
                              </div>
                              <div style={{fontSize:9,fontWeight:600,color:'#16A34A',marginTop:1}}>📦 Total DP Stock Value</div>
                            </div>
                          </div>
                        )}
                        ...
                        {!sparesLoading && sfSpares
                          .filter(s => sparesSearch.trim() ? true : parseFloat(s.stock) > 0)
                          .filter(s =>
                            !sparesSearch.trim() ||
                          ...
                          .sort((a, b) => (b.usageCount || 0) - (a.usageCount || 0))
                          .slice(0, sparesSearch.trim() ? undefined : 20)
                          .map(s => {
                              ...
                              <div style={{padding:'10px 12px',display:'flex',alignItems:'flex-start',gap:10}}>
                                <div style={{flex:1,minWidth:0}}>
                                  ...
                                  <div style={{display:'flex',alignItems:'center',gap:6,marginTop:5}}>
                                    ...
                                    {editingLocation === s.materialCode ? (
                                      <>
                                        <input autoFocus value={locationDraft} onChange={e=>setLocationDraft(e.target.value)}
                                          onKeyDown={e=>{if(e.key==='Enter')saveLocation(s.materialCode,locationDraft);if(e.key==='Escape'){setEditingLocation(null);setLocationDraft('');}}}
                                          placeholder="e.g. Bin 1"
                                          style={{fontSize:11,padding:'2px 7px',border:'1.5px solid #7C3AED',borderRadius:5,outline:'none',fontFamily:'monospace',width:100}}/>
                                        <button onClick={()=>saveLocation(s.materialCode,locationDraft)}
                                          style={{fontSize:10,padding:'2px 8px',background:'#7C3AED',color:'white',border:'none',borderRadius:5,cursor:'pointer'}}>✓</button>
                                        <button onClick={()=>{setEditingLocation(null);setLocationDraft('');}}
                                          style={{fontSize:10,padding:'2px 6px',background:'#F3F4F6',border:'none',borderRadius:5,cursor:'pointer'}}>✕</button>
                                      </>
                                    ) : (
                                      <span onClick={()=>{setEditingLocation(s.materialCode);setLocationDraft(s.location||'');}}
                                        style={{fontSize:10,fontWeight:600,color:s.location?'#374151':'#9CA3AF',cursor:'pointer',
                                          background:'#F9FAFB',padding:'2px 10px',borderRadius:5,border:'1px dashed #D1D5DB'}}>
                        ...
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginTop:12}}>
                          ...
                          <button onClick={()=>setShowQuotation(true)}
                            style={{padding:'10px',background:'linear-gradient(135deg,#059669,#047857)',color:'white',border:'none',borderRadius:9,fontSize:11,fontWeight:700,cursor:'pointer'}}>
                            📄 Quotation
                          </button>
                        </div>
                        ...
                        <div style={{marginTop:8}}>
                          <button onClick={async()=>{setShowQuotHistory(p=>!p);if(!showQuotHistory)await loadQuotHistory();}}
                            style={{width:'100%',padding:'9px',background:'white',border:'1.5px solid #D1FAE5',borderRadius:9,fontSize:11,fontWeight:600,color:'#047857',cursor:'pointer'}}>
                            {showQuotHistory ? '▲ Hide' : '📄 Show'} Quotation History
                          </button>
                          {showQuotHistory && (
                            <div style={{background:'white',borderRadius:10,marginTop:6,boxShadow:'0 2px 8px rgba(0,0,0,.06)',overflow:'hidden'}}>
                              {quotHistLoading && <div style={{padding:20,textAlign:'center',color:'#9CA3AF',fontSize:12}}>Loading…</div>}
                              {!quotHistLoading && quotHistory.length===0 && (
                                <div style={{padding:20,textAlign:'center',color:'#9CA3AF',fontSize:12}}>No quotations generated yet</div>
                              )}
                              {!quotHistLoading && quotHistory.map(q=>(
                                <div key={q.id} style={{padding:'10px 14px',borderBottom:'1px solid #F3F4F6',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                                  <div>
                                    <div style={{fontSize:11,fontWeight:700,fontFamily:'monospace',color:'#059669'}}>{q.quotNo}</div>
                                    <div style={{fontSize:10,color:'#374151',marginTop:1}}>{q.customer?.name}</div>
                                    <div style={{fontSize:9,color:'#9CA3AF'}}>{q.date} · Valid till {q.validUntil}</div>
                                  </div>
                                  <div style={{textAlign:'right'}}>
                                    <div style={{fontSize:12,fontWeight:700,fontFamily:'monospace',color:'#059669'}}>₹{fmtINR(q.total)}</div>
                                    <div style={{fontSize:9,color:'#9CA3AF'}}>{q.items?.length} item{q.items?.length!==1?'s':''}</div>
                                  </div>
                                </div>
                              ))}
                            </div>
                    ...
                    {/* ── Quotation builder overlay ── */}
                    {showQuotation && (
                      <div style={{background:'white',borderRadius:14,padding:16,boxShadow:'0 4px 20px rgba(0,0,0,.12)'}}>
                        {/* Quotation header */}
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
                          <div>
                            <div style={{fontSize:14,fontWeight:700,color:'#059669'}}>📄 New Quotation</div>
                            <div style={{fontSize:10,color:'#6B7280',marginTop:1}}>{sfTabSession?.sfName}</div>
                          </div>
                          <button onClick={()=>{setShowQuotation(false);setQuotItems([]);setQuotCust({name:'',mobile:'',address:'',gstin:''});setQuotNo('');}}
                            style={{background:'#F3F4F6',border:'none',padding:'5px 10px',borderRadius:7,fontSize:11,cursor:'pointer',color:'#374151'}}>
                            ✕ Cancel
                          </button>
                        </div>

                        {/* Quotation No + Validity */}
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
                          <div>
                            <label style={{fontSize:9,fontWeight:700,color:'#059669',textTransform:'uppercase',display:'block',marginBottom:3}}>Quotation No. (optional)</label>
                            <input value={quotNo} onChange={e=>setQuotNo(e.target.value)}
                              placeholder={'QT-' + (sfTabSession?.sfId||'') + '-XXXXXX'}
                              style={{width:'100%',padding:'7px 9px',border:'1.5px solid #E5E7EB',borderRadius:8,fontSize:11,outline:'none',fontFamily:'monospace'}}/>
                          </div>
                          <div>
                            <label style={{fontSize:9,fontWeight:700,color:'#059669',textTransform:'uppercase',display:'block',marginBottom:3}}>Valid For (Days)</label>
                            <select value={quotValidDays} onChange={e=>setQuotValidDays(e.target.value)}
                              style={{width:'100%',padding:'7px 9px',border:'1.5px solid #E5E7EB',borderRadius:8,fontSize:11,outline:'none',background:'white'}}>
                        ...
                        <div style={{background:'#F0FDF4',borderRadius:10,padding:12,marginBottom:12}}>
                          <div style={{fontSize:11,fontWeight:700,color:'#065F46',marginBottom:8}}>👤 Customer / Party Details</div>
                          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:7,marginBottom:7}}>
                            <div>
                              <label style={{fontSize:9,fontWeight:600,color:'#6B7280',textTransform:'uppercase',display:'block',marginBottom:2}}>Name *</label>
                              <input value={quotCust.name} onChange={e=>setQuotCust(p=>({...p,name:e.target.value}))}
                                placeholder="Customer / Company name"
                                style={{width:'100%',padding:'7px 8px',border:'1.5px solid #A7F3D0',borderRadius:7,fontSize:11,outline:'none'}}/>
                            </div>
                            <div>
                              <label style={{fontSize:9,fontWeight:600,color:'#6B7280',textTransform:'uppercase',display:'block',marginBottom:2}}>Mobile</label>
                              <input value={quotCust.mobile} onChange={e=>setQuotCust(p=>({...p,mobile:e.target.value}))}
                                placeholder="10-digit mobile"
                                style={{width:'100%',padding:'7px 8px',border:'1.5px solid #A7F3D0',borderRadius:7,fontSize:11,outline:'none',fontFamily:'monospace'}}/>
                            </div>
                          </div>
                          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:7}}>
                            <div>
                              <label style={{fontSize:9,fontWeight:600,color:'#6B7280',textTransform:'uppercase',display:'block',marginBottom:2}}>GSTIN (optional)</label>
                              <input value={quotCust.gstin} onChange={e=>setQuotCust(p=>({...p,gstin:e.target.value.toUpperCase()}))}
                                placeholder="15-char GSTIN"
                                style={{width:'100%',padding:'7px 8px',border:'1.5px solid #A7F3D0',borderRadius:7,fontSize:11,outline:'none',fontFamily:'monospace',letterSpacing:1}}/>
                            </div>
                            <div>
                              <label style={{fontSize:9,fontWeight:600,color:'#6B7280',textTransform:'uppercase',display:'block',marginBottom:2}}>Address</label>
                              <input value={quotCust.address} onChange={e=>setQuotCust(p=>({...p,address:e.target.value}))}
                        ...
                        {/* Item search / add */}
                        <div style={{marginBottom:10}}>
                          <label style={{fontSize:9,fontWeight:700,color:'#059669',textTransform:'uppercase',display:'block',marginBottom:4}}>🔍 Add Spare to Quotation</label>
                          <input value={quotSearchQ} onChange={e=>setQuotSearchQ(e.target.value)}
                            placeholder="Search by code or description…"
                            style={{width:'100%',padding:'8px 10px',border:'1.5px solid #A7F3D0',borderRadius:8,fontSize:12,outline:'none'}}/>
                          {quotSearchQ.trim() && (
                            <div style={{border:'1px solid #E5E7EB',borderRadius:8,marginTop:4,maxHeight:160,overflowY:'auto',boxShadow:'0 4px 12px rgba(0,0,0,.08)'}}>
                              {sfSpares
                                .sort((a, b) => (b.usageCount || 0) - (a.usageCount || 0))
                                .slice(0, 20)
                                .filter(s => s.materialCode.toLowerCase().includes(quotSearchQ.toLowerCase()) || s.description.toLowerCase().includes(quotSearchQ.toLowerCase()))
                                .map(s=>(
                                  <div key={s.materialCode} onClick={()=>addQuotItem(s)}
                                    style={{padding:'9px 12px',borderBottom:'1px solid #F3F4F6',cursor:'pointer',background:'white',display:'flex',justifyContent:'space-between',alignItems:'center'}}
                                    onMouseEnter={e=>e.currentTarget.style.background='#F0FDF4'}
                                    onMouseLeave={e=>e.currentTarget.style.background='white'}>
                                    <div>
                                      <span style={{fontFamily:'monospace',fontSize:10,color:'#059669',marginRight:6}}>{s.materialCode}</span>
                                      <span style={{fontSize:11,color:'#111827'}}>{s.description}</span>
                                      {parseFloat(s.stock) <= 0 && <span style={{fontSize:9,background:'#FEF2F2',color:'#DC2626',borderRadius:4,padding:'1px 5px',marginLeft:5}}>Out of Stock</span>}
                                    </div>
                                    <span style={{fontSize:11,fontWeight:600,color:'#059669',fontFamily:'monospace'}}>₹{fmtINR(s.mrp)}</span>
                                  </div>
                                ))}
                              {sfSpares.filter(s => s.materialCode.toLowerCase().includes(quotSearchQ.toLowerCase()) || s.description.toLowerCase().includes(quotSearchQ.toLowerCase())).length === 0 && (
                                <div style={{padding:12,textAlign:'center',color:'#9CA3AF',fontSize:11}}>No matching spare parts</div>
                              )}
                            </div>
                          )}
                        ...
                        {quotItems.length > 0 && (
                          <div style={{marginBottom:12}}>
                            <div style={{fontSize:11,fontWeight:700,color:'#374151',marginBottom:6}}>📋 Quotation Items</div>
                            <div style={{background:'#065F46',borderRadius:'8px 8px 0 0',display:'grid',gridTemplateColumns:'1fr 50px 60px 50px 20px',gap:4,padding:'6px 8px'}}>
                              {['Description','Qty','Rate','Disc.',''].map((h,i)=>(
                                <div key={i} style={{fontSize:9,fontWeight:600,color:'rgba(255,255,255,.7)',textAlign:i===1||i===2||i===3?'center':'left'}}>{h}</div>
                              ))}
                            </div>
                            {quotItems.map(item=>(
                              <div key={item.materialCode}
                                style={{display:'grid',gridTemplateColumns:'1fr 50px 60px 50px 20px',gap:4,padding:'7px 8px',borderBottom:'1px solid #F3F4F6',background:'white',alignItems:'center'}}>
                                <div>
                                  <div style={{fontSize:10,fontFamily:'monospace',color:'#059669'}}>{item.materialCode}</div>
                                  <div style={{fontSize:11,color:'#111827',lineHeight:1.3}}>{item.description}</div>
                                </div>
                                <input type="number" min="1" value={item.qty}
                                  onChange={e=>updateQuotLine(item.materialCode,'qty',e.target.value)}
                                  style={{width:'100%',padding:'4px 4px',border:'1.5px solid #E5E7EB',borderRadius:5,fontSize:11,textAlign:'center',outline:'none',fontFamily:'monospace'}}/>
                                <input type="number" min="0" value={item.rate}
                                  onChange={e=>updateQuotLine(item.materialCode,'rate',e.target.value)}
                                  style={{width:'100%',padding:'4px 4px',border:'1.5px solid #E5E7EB',borderRadius:5,fontSize:11,textAlign:'center',outline:'none',fontFamily:'monospace'}}/>
                                <input type="number" min="0" value={item.discount}
                                  onChange={e=>updateQuotLine(item.materialCode,'discount',e.target.value)}
                                  placeholder="0"
                                  style={{width:'100%',padding:'4px 4px',border:'1.5px solid #E5E7EB',borderRadius:5,fontSize:11,textAlign:'center',outline:'none',fontFamily:'monospace'}}/>
                                <button onClick={()=>removeQuotLine(item.materialCode)}
                                  style={{background:'none',border:'none',color:'#DC2626',fontSize:14,cursor:'pointer',padding:0,lineHeight:1}}>×</button>
                              </div>
                            ))}
                            ...
                            {(()=>{
                              const {subtotal,taxAmt,total}=calcQuotTotals();
                              return (
                                <div style={{background:'#F0FDF4',border:'1px solid #A7F3D0',borderRadius:'0 0 8px 8px',padding:'10px 12px'}}>
                                  <div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#374151',marginBottom:3}}>
                                    <span>Sub Total (excl. GST)</span><span style={{fontFamily:'monospace',fontWeight:600}}>₹{fmtINR(subtotal)}</span>
                                  </div>
                                  <div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#374151',marginBottom:6}}>
                                    <span>GST @ 18% (indicative)</span><span style={{fontFamily:'monospace',fontWeight:600}}>₹{fmtINR(taxAmt)}</span>
                                  </div>
                                  <div style={{display:'flex',justifyContent:'space-between',fontSize:14,fontWeight:700,color:'#059669',borderTop:'1px solid #A7F3D0',paddingTop:6}}>
                                    <span>QUOTED TOTAL</span><span style={{fontFamily:'monospace'}}>₹{fmtINR(total)}</span>
                                  </div>
                                  <div style={{fontSize:9,color:'#6B7280',marginTop:4}}>Valid for {quotValidDays} days · GST shown is indicative · No stock deducted</div>
                                </div>
                              );
                            })()}
                          </div>
                        ...
                        <button onClick={generateQuotationPdf}
                          style={{width:'100%',padding:'12px',background:'linear-gradient(135deg,#059669,#047857)',color:'white',border:'none',borderRadius:10,fontSize:13,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}>
                          <svg width="16" height="16" fill="none" stroke="white" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 10v6m0 0l-3-3m3 3l3-3M3 17v3a1 1 0 001 1h16a1 1 0 001-1v-3"/></svg>
                          Generate & Download Quotation PDF
                        </button>
                        <div style={{marginTop:8,textAlign:'center',fontSize:10,color:'#6B7280'}}>
                          ℹ️ Quotation does not deduct stock — only Tax Invoice deducts stock.
                        </div>
                      </div>
                    )}
                  </div>
